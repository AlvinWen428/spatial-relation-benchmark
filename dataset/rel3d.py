...
class Rel3dDataset(Dataset):
    def __init__(self, split, predicate_dim, object_dim, data_path, load_img,
        ...
        print('%d relations in %s' % (len(self.relations), self.split))
        num_pos = 0
        num_neg = 0
        self.num_rel_pos = defaultdict(int)
        self.num_rel_neg = defaultdict(int)
        for sample in self.relations:
            if sample['label']:
                num_pos += 1
                self.num_rel_pos[sample['predicate']] += 1
            else:
                num_neg += 1
                self.num_rel_neg[sample['predicate']] += 1
        self.all_rel = list(set(self.num_rel_neg) | set(self.num_rel_pos))
        assert set(self.all_rel) == set(self.predicates)

        print(f"Percentage of positive labels  in {self.split}:"
              f" {round(num_pos / (num_neg + num_pos), 3)}")
        print(f"Percentage of negative labels  in {self.split}:"
              f" {round(num_neg / (num_neg + num_pos), 3)}")
        print({
            x: (self.num_rel_pos[x], self.num_rel_neg[x],
                round(self.num_rel_pos[x] / (self.num_rel_pos[x] + self.num_rel_neg[x]), 3))
            for x in self.all_rel
        })
    ...
    def __getitem__(self, idx, visualize=False):
        ...
        if self.load_img:
            ...
            example['img'] = img_crop
            example['depth'] = depth_crop
