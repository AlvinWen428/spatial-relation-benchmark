...
class Block(nn.Module):
    ...
    def forward(self, x, attn_mask=None, rel_pos_bias=None):
        x_attn_output = self.attn(self.norm1(x), attn_mask=attn_mask, rel_pos_bias=rel_pos_bias)

        if self.init_values is None:
            x = x + self.drop_path(x_attn_output)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(
                self.gamma_1 * x_attn_output
            )
...
class VitModel(VisionTransformer):
    def __init__(self,
                 pretrained="",
        ...
        super(VitModel, self).__init__(*args, **kwargs)
        self.cls_token = None
        self.head = None
    ...
    def forward(self, x):
        x = self.forward_features(x)
        x = rearrange(x, "b (h w) c -> b c h w", h=self.patch_embed.grid_size[0], w=self.patch_embed.grid_size[1],)
        return x
...
def vit_small(pretrain_ckp=""):
    return VitModel(img_size=224, patch_size=16, in_chans=3, embed_dim=384, depth=12, num_heads=6,
                    pretrained=pretrain_ckp)


def vit_base(pretrain_ckp=""):
    return VitModel(img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12,
                    pretrained=pretrain_ckp)


def vit_large(pretrain_ckp=""):
    return VitModel(img_size=224, patch_size=16, in_chans=3, embed_dim=1024, depth=24, num_heads=16,
                    pretrained=pretrain_ckp)
